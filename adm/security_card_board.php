...
<!-- 게시판 접근 권한 정책 -->
<div class="card">
    <div class="card-header" onclick="toggleCard('board-card')">
        🔐 게시판 접근 권한 정책 <span id="board-toggle">▶</span>
    </div>
    <div class="card-content" id="board-card">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                        <div style="flex: 1;">
                            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 5px;">
                                <span class="text-title-bold"><?php echo $analysis['icon']; ?> <?php echo htmlspecialchars($board['bo_subject']); ?></span>
                                <span class="badge-gray" style="font-family: monospace;">
                            ...
                            <div class="text-description">
                                설정된 접근 권한:
                                <span class="badge-level">목록(<?php echo $board['bo_list_level']; ?>)</span>
                                <span class="badge-level">읽기(<?php echo $board['bo_read_level']; ?>)</span>
                                <span class="badge-level">쓰기(<?php echo $board['bo_write_level']; ?>)</span>
                                <span class="badge-level">답글(<?php echo $board['bo_reply_level']; ?>)</span>
                                <span class="badge-level">댓글(<?php echo $board['bo_comment_level']; ?>)</span>
                            </div>

                            <?php if (!empty($analysis['warnings'])): ?>
                            <div style="margin-top: 8px;">
                                <?php foreach ($analysis['warnings'] as $warning): ?>
                                <div class="<?php echo $analysis['status'] == 'exception' ? 'alert-warning-sm' : ($analysis['status'] == 'safe' ? 'alert-success-sm' : 'alert-danger-sm'); ?>">
                                    <?php echo $warning; ?>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            ...
                            <div class="alert-success-sm" style="padding: 8px 12px; margin-top: 8px;">
                            ...
                            <div style="margin-top: 10px; display: flex; gap: 3px; flex-wrap: wrap;">
                                ...
                                <button onclick="updateBoardPermissions('<?php echo $board['bo_table']; ?>', 'fix_member_level', <?php echo $analysis['default_member_level']; ?>)"
                                        class="btn-success-sm" style="margin-right: 3px;">
                                ...
                                <button onclick="toggleBoardException('<?php echo $board['bo_table']; ?>')"
                                        class="btn-secondary-sm" style="margin-right: 3px;">
                                    예외 처리 해제
                                </button>
                                <?php elseif ($analysis['has_guest_permissions'] && $analysis['status'] != 'safe'): ?>
                                <button onclick="toggleBoardException('<?php echo $board['bo_table']; ?>')"
                                        class="btn-warning-sm" style="margin-right: 3px;">
                                    예외 처리
                                </button>
                                <?php endif; ?>
                            </div>
