# GnuKeeper - Gnuboard5 보안 플러그인 개발 프로젝트

## 프로젝트 개요

**앱 이름**: GnuKeeper
**목적**: gnuboard5를 위한 종합적인 보안 플러그인을 개발합니다. WordPress의 보안 플러그인과 같이 일반 사용자가 쉽게 사이트를 보호할 수 있도록 다양한 보안 기능을 제공합니다.
...
### 네이밍 규칙
- **앱 이름**: GnuKeeper
- **함수 접두사**: 커스텀 전역 함수 선언 시 `gk_` 접두사 사용
  - 예: `gk_get_security_config()`, `gk_create_tables()`
- **클래스 접두사**: 커스텀 전역 클래스 선언 시 `GK_` 접두사 사용
  - 예: `GK_SecurityManager`, `GK_IPBlocker`
- **상수 접두사**: 커스텀 전역 상수 선언 시 `GK_` 접두사 사용
  - 예: `GK_VERSION`, `GK_PLUGIN_PATH`
- **기타**: `data/dbconfig.php` 등을 참고하여 기존 네이밍 규칙을 존중한다.
...
1. Phase 1: 관리자 인터페이스 기본 구조
2. Phase 2: 기본 접근제어 및 IP 차단 기능 확장
